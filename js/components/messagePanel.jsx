...
export default function MessagePanel({ channelId, encryptFunction, colors }) {
    ...
    const sendMessage = () => {
        const encrMsg = encryptFunction.function(messageInput, vKey, true);
    ...
    return (<div className="message-panel">
        <div className="key-control-cont">
            <label htmlFor="vKeyInput">{encryptFunction.name} key: </label>
        ...
        <div id="msgBox" className="message-box" style={{ backgroundColor: colors[0], border: "2px solid " + colors[1] }}>
            {messages.map(msg => {
                return <Message key={msg[0]} vKey={vKey} encryptFunction={encryptFunction} id={msg[0]} message={msg[1]} color={colors[0]} bgColor={colors[1]} />
            })}
        </div>
